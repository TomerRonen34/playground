...
def merge_gdfs_by_biggest_intersection(left_gdf: GeoDataFrame, right_gdf: GeoDataFrame) -> GeoDataFrame:
    """
    For every row in left_gdf, joins the row of right_gdf which has the biggest intersection with it.
    Also adds an "index_right" column to document which row of right_gdf was chosen.

    Geometries in left_gdf that have no intersections in right_gdf are joined with NaN values,
